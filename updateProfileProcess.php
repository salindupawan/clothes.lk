...
if (isset($_SESSION["u"])) {
    ...
    if (isset($_FILES["image"])) {
        ...
        if (!in_array($file_ex, $allowed_image_extentions)) {
            ...
            $image_rs = Database::search("SELECT * FROM `user_image` WHERE 
            `user_email`='" . $_SESSION["u"]["email"] . "'");
            $image_num = $image_rs->num_rows;

            if ($image_num == 1) {

                Database::iud("UPDATE `user_image` SET `path`='" . $file_name . "' WHERE 
                `user_email`='" . $_SESSION["u"]["email"] . "'");
            } else {

                Database::iud("INSERT INTO `user_image` (`path`,`user_email`) VALUES 
